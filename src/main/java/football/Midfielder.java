    ...
    public int decision_ball(Pitch pitch, Ball ball, Team team, int event, double con){
        if(event==1){
            ...
            if(Math.abs(getPlace().getLength()-modY)>=2){
                if(action<75) event=player_passing(recipient(team),ball,event,con);
                else player_dribbling(pitch,ball,con);
            }
            else if(getPlace().getWidth()==0 || getPlace().getWidth()==4){
                if(action<70) event=player_passing(recipient(team),ball,event,con);
                else player_dribbling(pitch,ball,con);
            }
                ...
                if(action<45) event=player_passing(recipient(team),ball,event,con);
